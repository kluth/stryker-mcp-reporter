...
export class MutationTrendTracker {
  ...
  public getTrendSummary(): { scoreDelta: number; isImproving: boolean; latestScore: number } {
    ...
    if (this.history.length === 1) {
      return { scoreDelta: 0, isImproving: true, latestScore: latest.mutationScore };
    }

    const previous = this.history[this.history.length - 2];
    const scoreDelta = Number((latest.mutationScore - previous.mutationScore).toFixed(2));
