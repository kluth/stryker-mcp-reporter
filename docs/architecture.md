...
- `src/infrastructure/database`: Adapters for local SQLite storage (persisting runs and history caching).
...
4. **Data Stream & Storage**: The parsed `MutationReport` is pushed to a singleton `ReportStream` and persisted to the local **SQLite database** for historical tracking and quick retrieval.
5. **Follow-up**: When the AI agent asks for a Risk Score (`get_risk_score`) or for Mutant Insights (`why_is_this_bad`), the respective Use Cases pull the latest report from the `ReportStream` (or SQLite cache) and process it using the domain logic.
